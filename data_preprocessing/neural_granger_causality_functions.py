...
def train_model_ista(clstm, X, context, lr, max_iter, lam=0, lam_ridge=0,
    ...
    for it in range(max_iter):
        ...
        if (it + 1) % check_every == 0:
            ...
            params.append(GC.cpu().data.numpy())
    ...
    return train_loss_list, params


def train_unregularized(clstm, X, X_val, context, lr, max_iter, lookback=5,
                        check_every=50, verbose=1, lam_ridge=0):
    '''Train model with Adam.'''
    p = X.shape[-1]
    loss_fn = nn.MSELoss(reduction='mean')
    optimizer = torch.optim.Adam(clstm.parameters(), lr=lr, weight_decay=lam_ridge)
    train_loss_list = []
    val_loss_list = []
    ...
    X_val, Y_val = zip(*[arrange_input(x, context) for x in X_val])
    ...
    for it in range(max_iter):
        ...
        if (it + 1) % check_every == 0:
            ...
            #validation metrics
            p_val = X_val.shape[-1]
            pred_val, hidden_val = clstm(X_val)
            val_loss = sum([loss_fn(pred_val[:, :, k], Y_val[:, :, k]) for k in range(p_val)])

            #val_loss_list.append(val_loss.detach().numpy()[np.newaxis][0]/p_val)
            mean_val_loss = val_loss/p_val
            val_loss_list.append(mean_val_loss.detach())
...
def ith_fold_tensors(i, all_dnas, all_mtvols, all_cellvols):
    ith_fold_dnas = np.delete(all_dnas, i, axis=0)
    ith_fold_mtvols = np.delete(all_mtvols, i, axis=0)
    ith_fold_cellvols = np.delete(all_cellvols, i, axis=0)

    ith_val_dna = all_dnas[i]
    ith_val_mtvol = all_mtvols[i]
    ith_val_cellvol = all_cellvols[i]

    scaled_ith_fold_dna = (ith_fold_dnas - np.min(ith_fold_dnas))/(np.max(ith_fold_dnas) - np.min(ith_fold_dnas))
    scaled_ith_fold_mtvols = (ith_fold_mtvols - np.min(ith_fold_mtvols))/(np.max(ith_fold_mtvols) - np.min(ith_fold_mtvols))
    scaled_ith_fold_cellvols = (ith_fold_cellvols - np.min(ith_fold_cellvols))/(np.max(ith_fold_cellvols) - np.min(ith_fold_cellvols))

    scaled_ith_val_dna = (ith_val_dna - np.min(ith_fold_dnas))/(np.max(ith_fold_dnas) - np.min(ith_fold_dnas))
    scaled_ith_val_mtvols = (ith_val_mtvol - np.min(ith_fold_mtvols))/(np.max(ith_fold_mtvols) - np.min(ith_fold_mtvols))
    scaled_ith_val_cellvols = (ith_val_cellvol - np.min(ith_fold_cellvols))/(np.max(ith_fold_cellvols) - np.min(ith_fold_cellvols))

    ith_fold_X = torch.tensor(np.array([scaled_ith_fold_dna, scaled_ith_fold_mtvols, scaled_ith_fold_cellvols]))
    ith_fold_X = ith_fold_X.transpose(0,2)
    ith_fold_X = ith_fold_X.transpose(0,1)

    ith_val_X = torch.tensor(np.array([scaled_ith_val_dna, scaled_ith_val_mtvols, scaled_ith_val_cellvols])[np.newaxis])
    ith_val_X = ith_val_X.transpose(1,2)

    return ith_fold_X, ith_val_X


def train_val_ith_fold_unregularised(all_dnas, all_mtvols, all_cellvols, net, fold, lr, num_iter):
    """
    input: [i, sparsity, lr, num_iter, folder_name]
    """
    ith_fold_X, ith_val_X = ith_fold_tensors(fold, all_dnas, all_mtvols, all_cellvols)

    if net == 1:
        sparsity = torch.tensor(np.array([[1,0,0],
                                          [0,1,0],
                                          [0,0,1]])).bool()
                                          
    elif net == 2:
        sparsity = torch.tensor(np.array([[1,1,0],
                                          [0,1,0],
                                          [0,1,1]])).bool()
        ...
        sparsity = torch.tensor(np.array([[1,1,1],
                                          [1,1,1],
                                          [1,1,1]])).bool()

    clstm = cLSTMSparse(ith_fold_X.shape[-1], sparsity = sparsity, hidden=5)

    train_loss_list, val_loss_list = train_unregularized(
        clstm, ith_fold_X, ith_val_X, context=6, lr=lr, max_iter=num_iter,
        check_every=50, verbose = 0, lam_ridge=0)
